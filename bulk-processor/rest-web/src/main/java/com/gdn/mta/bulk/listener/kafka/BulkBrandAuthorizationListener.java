...
@Slf4j
@Service
public class BulkBrandAuthorizationListener {
  @Autowired
  ...
  @KafkaListener(topics = "#{kafkaTopicProperties.getBrandAuthorisationBulkProcess()}", autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    log.info("onDomainEventConsumed : {} : {} ", kafkaTopicProperties.getBrandAuthorisationBulkProcess(), message);
    try {
      ...
      internalProcessServiceWrapper.processBulkBrandAuthorisationEvent(internalProcessDataDomainEventModel.getStoreId(),
      ...
      log.info("error caught on onDomainEventConsumed. event : {} ",
          kafkaTopicProperties.getBrandAuthorisationBulkProcess(), e);
    }
  }
