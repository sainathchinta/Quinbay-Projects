...
public class BulkConvertedFileProductCreationListener {
  @Autowired
  ...
  @KafkaListener(topics = "#{kafkaTopicProperties.getBulkConvertedProductCreationEvent()}", autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    log.info("Consume event {} with message {} ", kafkaTopicProperties.getBulkConvertedProductCreationEvent(), message);
    ...
    try {
      ...
      log.error("Error while consuming event : {}, message : {} ",
          kafkaTopicProperties.getBulkConvertedProductCreationEvent(), message, e);
