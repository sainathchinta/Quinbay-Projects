...
@Service
@Slf4j
public class BulkDeleteInstantPickupItemListener {
  ...
  @KafkaListener(topics = "#{kafkaTopicProperties.getBulkDeleteInstantPickupItemEvent()}", autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    log.info("Received event for topic : {} with message : {}",
        kafkaTopicProperties.getBulkDeleteInstantPickupItemEvent(), message);
    try {
      ...
      bulkDeleteService.processEvent(bulkUpdateEventModel);
    } catch (Exception e) {
      log.error("Error while process bulk delete instant pickup event with payload : {}, error - ", message, e);
      throw new ApplicationException(ErrorCategory.COMMUNICATION_FAILURE,
          "Error while listening and processing " + "BULK_DELETE_INSTANT_PICKUP_ITEM_EVENT bulkUpdateEventModel : "
