...
@Slf4j
@Service
public class BulkImageDownloadListener {
  ...
  @KafkaListener(topics = "#{kafkaTopicProperties.getBulkCreateDownloadImageEvent()}", autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    ...
    log.info("Download image com.gdn.mta.bulk.create.download.image with payload ", message);
    try {
      ...
      log.error("Error when listening to event com.gdn.mta.bulk.create.download.image message : {} ", message, e);
    }
  }
}
