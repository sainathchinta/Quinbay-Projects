...
@Slf4j
@Service
public class BulkPriceUpdateNewListener {
  ...
  @KafkaListener(topics = "#{kafkaTopicProperties.getBulkPriceUpdateNewEvent()}", autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    log.info("onDomainEventConsumed : {} : {} ", kafkaTopicProperties.getBulkPriceUpdateNewEvent(), message);
    try {
      ...
      internalProcessServiceWrapper.processBulkInternalBulkPriceUpdate(internalProcessDataDomainEventModel, true);
    } catch (Exception e) {
      log.info("Error caught on onDomainEventConsumed. event : {} ", kafkaTopicProperties.getBulkPriceUpdateNewEvent(), e);
    }
  }
