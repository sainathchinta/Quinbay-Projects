...
@Service
@Slf4j
public class BulkProcessGenericPriorityQueueListener {
  ...
  @KafkaListener(topics = "#{kafkaTopicProperties.getBulkGenericCreatePriorityEvent()}", autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    log.info("Consume event {} with message {} ", kafkaTopicProperties.getBulkGenericCreatePriorityEvent(), message);
    ...
    try {
      ...
      log.error("Error while consuming event : {}, message : {} ",
          kafkaTopicProperties.getBulkGenericCreatePriorityEvent(), message, e);
