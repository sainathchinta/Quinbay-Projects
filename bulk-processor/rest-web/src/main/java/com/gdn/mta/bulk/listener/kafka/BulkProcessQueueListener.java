 ...
 */
@Service
public class BulkProcessQueueListener {

  private static final Logger LOG = LoggerFactory.getLogger(BulkProcessQueueListener.class);
  ...
  @KafkaListener(topics = "#{kafkaTopicProperties.getBulkCreateEvent()}", autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    LOG.info("Consume event {} with message {} ", kafkaTopicProperties.getBulkCreateEvent(), message);
    ...
    try {
      ...
      ProcessorService processorService = (ProcessorService) autowireCapableBeanFactory.getBean(bulkProcessQueue
          .getBulkProcessType() + Constant.PROCESSOR_SERVICE);
      ...
      if (PRODUCT_LEVEL_3.equalsIgnoreCase(bulkProcessQueue.getBulkProcessType())) {
        this.trackerService.sendTracker(TrackerConstants.PRODUCT_CREATE_EVENT, TrackerConstants
            .CREATE_FLOW1_WEB, TrackerConstants.SUBMIT, TrackerConstants.FAILED,
      ...
      throw new ApplicationException(ErrorCategory.COMMUNICATION_FAILURE, "Error while listening and processing " +
          "bulkProcessQueue : " + bulkProcessQueue.toString() + ", error is : " + e.getMessage(), e);
    }
  }
}
