...
@Service
public class BulkSuspensionEventListener {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(BulkSuspensionEventListener.class);

  @Trace(dispatcher = true)
  @KafkaListener(topics = "#{kafkaTopicProperties.getBulkProductSuspensionEvent()}", autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    LOG.info("Consume event {} with message {} ", kafkaTopicProperties.getBulkProductSuspensionEvent(), message);
    ...
    try {
      loggerModel = new LoggerAttributeModel(this, "onDomainEventConsumed", null, null, null, null, null,
          LoggerChannel.KAFKA.getValue(), kafkaTopicProperties.getBulkProductSuspensionEvent(),
      ...
      bulkProductSuspensionService.processSuspensionEvent(bulkUpdateEventModel);
    } catch (Exception e) {
      LOG.error(
      ...
      throw new ApplicationException(ErrorCategory.COMMUNICATION_FAILURE,
          "Error while listening and processing " + "BULK_PRODUCT_SUSPENSION_EVENT bulkUpdateEventModel : "
