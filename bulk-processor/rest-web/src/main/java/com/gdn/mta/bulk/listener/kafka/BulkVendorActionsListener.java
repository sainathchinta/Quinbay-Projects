...
@Slf4j
@Service
public class BulkVendorActionsListener {
  @Autowired
  ...
  @KafkaListener(topics = "#{kafkaTopicProperties.getBulkApprovalRejectionProcessEvent()}", autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    log.info("onDomainEventConsumed : {} : {} ", kafkaTopicProperties.getBulkApprovalRejectionProcessEvent(), message);
    try {
      ...
      internalProcessServiceWrapper.processBulkVendorActionsEvent(internalProcessDataDomainEventModel.getStoreId(),
      ...
      log.info("error caught on onDomainEventConsumed. event : {} ",
          kafkaTopicProperties.getBulkApprovalRejectionProcessEvent(), e);
    }
  }
