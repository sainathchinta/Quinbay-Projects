...
public interface OrderFeign {
  ...
  GdnRestListResponse<OrderItemSummaryResponse> findOrderItemSummaryByFilter(@Param("storeId") String storeId,
  ...
  @RequestLine("POST /api/summary/bulk-download?storeId={storeId}&channelId={channelId}&clientId={clientId}"
      + "&requestId={requestId}&username={username}&page={page}&size={size}")
  @Headers({"Content-Type: application/json", "Accept: application/json"})
  GdnRestListResponse<OrderItemSummaryResponse> getOrderItemsForBulkDownload(@Param("storeId") String storeId,
      @Param("channelId") String channelId, @Param("clientId") String clientId, @Param("requestId") String requestId,
