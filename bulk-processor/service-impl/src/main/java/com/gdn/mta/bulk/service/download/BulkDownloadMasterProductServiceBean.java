...
@Service(value = "bulkMasterProductDataServiceBean")
public class BulkDownloadMasterProductServiceBean implements BulkProcessDataService {

  private static final Logger LOGGER = LoggerFactory.getLogger(BulkDownloadMasterProductServiceBean.class);
  ...
  public BulkDataResponse getData(BulkDownloadRequest request) throws Exception {
    LOGGER.info("Calling Get Data for master product data for requestId {}, BulkProcessEntity {}",
        request.getRequestId(), request.getBulkProcessEntity());
    MasterProductDownloadRequest masterProductDownloadRequest = (MasterProductDownloadRequest) request;
    List<MasterProductResponse> masterProductResponseList =
        activeProductRepository.getActiveProductDetails(masterProductDownloadRequest);
