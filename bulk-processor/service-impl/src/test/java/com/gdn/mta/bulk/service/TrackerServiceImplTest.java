...
public class TrackerServiceImplTest {
  ...
  private static String EVENT = "product-creation";
  private static String ATTR_TYPE = "flow-1-api";
  private static String ATTR_NAME = "submit";
  private static String ATTR_VALUE = "-";
  private static String CUSTOM_FIELD = "username@mail.com";
  ...
  @Test
  public void sendTracker_Valid_Success() throws Exception {
    trackerService.sendTracker(EVENT, ATTR_TYPE, ATTR_NAME, ATTR_VALUE, CUSTOM_FIELD);
    Mockito.verify(kafkaProducer).send(
        Mockito.eq(kafkaTopicProperties.getAnalyticBwaEventPublish()),Mockito.any());
    Mockito.verify(kafkaTopicProperties, times(2)).getAnalyticBwaEventPublish();
  }
  ...
  public void trackProductCreationFailureTest() {
    ...
    verify(kafkaProducer).send(Mockito.eq(DomainEventName.PRODUCT_CREATION_FAILURE), productCreationFailureDomainEventModelCaptor.capture());
  }
  ...
  public void trackProductCreationFailureTestNoCategoriesNoAttributesNoItemsProductRequest() {
    trackerService.trackProductCreationFailure(REQUEST_ID, new ProductCreationRequest(), ERROR_MESSAGE);
    verify(kafkaProducer).send(Mockito.eq(DomainEventName.PRODUCT_CREATION_FAILURE), productCreationFailureDomainEventModelCaptor.capture());
  }
  ...
  public void trackProductCreationFailureTestNoProductItemAttributeValuesRequest() {
    ...
    verify(kafkaProducer).send(
        Mockito.eq(DomainEventName.PRODUCT_CREATION_FAILURE), productCreationFailureDomainEventModelCaptor.capture());
  }
}
