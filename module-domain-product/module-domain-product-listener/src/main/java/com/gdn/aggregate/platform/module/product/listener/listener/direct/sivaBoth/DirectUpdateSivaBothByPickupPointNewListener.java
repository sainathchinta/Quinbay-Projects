...
@Component
@ConditionalOnProperty(name = Enabler.FLOW_PICKUP_POINT_CHANGE_COMBINED_EVENT, havingValue = "true")
public class DirectUpdateSivaBothByPickupPointNewListener {
  ...
  public void onPickupPointEventSivaBoth(ConsumerRecord<String, String> record) {
    log.info("Consumed new pick point data collection update event {} , Topic : {} ", record, record.topic());
    listenerService.onPickupPointNewEvent(record, false, false, TraceHelper.getTraceId(tracer));
  }
  ...
  public void onAllPickupPointEventSivaBoth(ConsumerRecord<String, String> record) {
    log.info("Consumed new pick point data agp collection update event {}, Topic : {} ", record, record.topic());
    listenerService.onPickupPointNewEvent(record, true, true, TraceHelper.getTraceId(tracer));
  }
}
