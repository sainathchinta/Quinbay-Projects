...
public class GfsToGcsFinalImageMigrationListenerTest {
  private static final String STORE_ID = "10001";
  private static final String PRODUCT_CODE = "productCode";

  @InjectMocks
  private GfsToGcsFinalImageMigrationListener gfsToGcsFinalImageMigrationListener;
  ...
  public void onDomainEventConsumedTest() throws IOException {
    ...
    Mockito.doNothing().when(productServiceWrapper).migrateFinalImagesFromGfsToGcsForProducts(STORE_ID, PRODUCT_CODE);
    gfsToGcsFinalImageMigrationListener.onDomainEventConsumed(payload);
    Mockito.verify(objectMapper).readValue(payload, CommonImageBackfillingEventModel.class);
    Mockito.verify(productServiceWrapper).migrateFinalImagesFromGfsToGcsForProducts(STORE_ID, PRODUCT_CODE);
  ...
  public void onDomainEventConsumedErrorTest() throws IOException {
    Mockito.when(objectMapper.readValue(payload, CommonImageBackfillingEventModel.class))
        .thenReturn(commonImageBackfillingEventModel);
    Mockito.doThrow(ApplicationRuntimeException.class).when(productServiceWrapper).migrateFinalImagesFromGfsToGcsForProducts(STORE_ID, PRODUCT_CODE);
    gfsToGcsFinalImageMigrationListener.onDomainEventConsumed(payload);
    Mockito.verify(objectMapper).readValue(payload, CommonImageBackfillingEventModel.class);
    Mockito.verify(productServiceWrapper).migrateFinalImagesFromGfsToGcsForProducts(STORE_ID, PRODUCT_CODE);
  }
