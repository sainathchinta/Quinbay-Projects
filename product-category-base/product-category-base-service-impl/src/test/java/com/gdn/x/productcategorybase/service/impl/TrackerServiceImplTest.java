...
public class TrackerServiceImplTest {
  ...
  @Captor
  private ArgumentCaptor<ProductCreationFailureDomainEventModel> productCreationFailureDomainEventModelCaptor;
  ...
  public void init() {
    ...
    productRequest =
        new ProductRequest.Builder().productCode(PRODUCT_CODE).name(PRODUCT_NAME).length(LENGTH)
            ...
            .description(DESCRIPTION).brand(BRAND_NAME)
            .uniqueSellingPoint(UNIQUE_SELLING_POINT).uom(UOM).storeId(STORE_ID).promoSKU(false)
            .build();
    ...
    List<ProductItemAttributeValueRequest> itemAttributeValueRequests1 = new ArrayList<>();
    itemAttributeValueRequests1.add(new ProductItemAttributeValueRequest(attributeRequest1, VALUE, STORE_ID));
    ProductItemRequest itemRequest2 =
        new ProductItemRequest(PRODUCT_ITEM_NAME_2, UPC_CODE_2, SKU_CODE_2, false, false, STORE_ID);
    itemRequest2.setId("2");
    itemRequest2.setProductItemAttributeValues(itemAttributeValueRequests1);
  ...
  public void trackProductCreationFailureTest() {
    trackerService.trackProductCreationFailure(REQUEST_ID, productRequest, ERROR_MESSAGE);
    verify(domainEventPublisherService).publishProductFailure(productCreationFailureDomainEventModelCaptor.capture());
  }

  @Test
  public void trackProductCreationFailureTestNullProductRequest() {
    trackerService.trackProductCreationFailure(REQUEST_ID, null, ERROR_MESSAGE);
    verify(domainEventPublisherService).publishProductFailure(productCreationFailureDomainEventModelCaptor.capture());
  }

  @Test
  public void trackProductCreationFailureTestNoCategoriesNoAttributesNoItemsProductRequest() {
    trackerService.trackProductCreationFailure(REQUEST_ID, new ProductRequest(), ERROR_MESSAGE);
    verify(domainEventPublisherService).publishProductFailure(productCreationFailureDomainEventModelCaptor.capture());
  }
  ...
  @Test
  public void trackProductCreationFailureTestNoAttributeWithProductAttributesProductRequest() {
  ...
  @Test
  public void trackProductCreationFailureTestNoProductAttributeValuesRequestsProductRequest() {
