...
@Service
public class Level2InventoryMinimumStockAlertSubscriber {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(Level2InventoryMinimumStockAlertSubscriber.class);
  ...
  @Autowired
  @Qualifier("level2InventoryBelowMinimumStockEventStoreKafkaMessageProcessor")
  private EventStoreKafkaMessageProcessor<Level2InventoryMinimumStockAlertEvent> eventStoreKafkaMessageProcessor;

  private String itemName;

  @Trace(dispatcher=true)
  @KafkaListener(topics = DomainEventName.STOCK_MINIMUM_STOCK_ALERT_NAME, autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
    ...
    try {
      if (this.validateMessageEvent(messageEvent)) {
        if(Objects.isNull(messageEvent.getPickupPointCode())) {
          this.productStockAlertService.updateMinimumStock(messageEvent, itemName, 0);
          this.productLevel3AggregatorService.updateMinimumStock(messageEvent, true, 0);
        }
        else{
          this.productStockAlertService.updateMinimumStock(messageEvent, itemName, 0);
        }
        this.eventStoreKafkaMessageProcessor.process(messageEvent, DomainEventName.STOCK_MINIMUM_STOCK_ALERT_NAME);
      }
    } catch (Exception e) {
      LOGGER.error(
          "Level2InventoryMinimumStockAlertEvent stockEventConsumed ERROR topic STOCK_MINIMUM_STOCK_ALERT_NAME : {}, Exception : {}",
          messageEvent, e);
    }
  }
  ...
  private boolean validateMessageEvent(Level2InventoryMinimumStockAlertEvent messageEvent)
      throws Exception {
    LOGGER.info("Received message stockEventConsumed from topic : {} with message : {}", DomainEventName.STOCK_MINIMUM_STOCK_ALERT_NAME, messageEvent);
    if (StringUtils.isNotEmpty(messageEvent.getGdnSku())) {
      ...
      if (StringUtils.isNotEmpty(itemName)) {
        ...
        MDC.put(GdnMandatoryRequestParameterUtil.CHANNEL_ID_KEY_PARAMETER, Level2InventoryMinimumStockAlertSubscriber.class.getName());
        MDC.put(GdnMandatoryRequestParameterUtil.CLIENT_ID_KEY_PARAMETER, Level2InventoryMinimumStockAlertSubscriber.class.getName());
        MDC.put(GdnMandatoryRequestParameterUtil.REQUEST_ID_KEY_PARAMETER, UUID.randomUUID().toString());
        MDC.put(GdnMandatoryRequestParameterUtil.USERNAME_KEY_PARAMETER, DomainEventName.STOCK_MINIMUM_STOCK_ALERT_NAME);
        MDC.put(GdnMandatoryRequestParameterUtil.AUTHENTICATOR_KEY, DomainEventName.STOCK_MINIMUM_STOCK_ALERT_NAME);
        return true;
      } else {
