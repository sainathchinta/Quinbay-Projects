...
@Service
@Slf4j
public class ImageQcBacklogKafkaSubscriberBean {
  ...
  @KafkaListener(topics = DomainEventName.IMAGE_QC_PREDICTION_BACKLOG_REQUEST, autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) throws Exception {
    ...
    log.info("Consume com.gdn.image.qc.backlog.prediction.request with request: {}", imageQcResponseDomainEvent);
    try {
      setMandatoryParameters();
      productServiceWrapper
          .processImageQcForBacklogProducts(MDC.get(GdnMandatoryRequestParameterUtil.STORE_ID_KEY_PARAMETER),
              imageQcResponseDomainEvent);
      log.info("Image qc backlog processing is done : {}", imageQcResponseDomainEvent.getProductCode());
    } catch (Exception e) {
      log.error(
          "Exception caught while processing event com.gdn.image.qc.backlog.prediction.request for productCode : {} ",
