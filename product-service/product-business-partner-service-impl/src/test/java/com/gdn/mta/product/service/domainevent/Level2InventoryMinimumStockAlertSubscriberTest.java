...
public class Level2InventoryMinimumStockAlertSubscriberTest {
  private static final String PICKUP_POINT_CODE = "pickupPointCode";

  @InjectMocks
  private Level2InventoryMinimumStockAlertSubscriber level2InventoryMinimumStockAlertSubscriber;
  ...
  public void testOnDomainEventConsumed_PP_NullTest() throws Exception {
    ...
    this.level2InventoryMinimumStockAlertSubscriber.onDomainEventConsumed(message);
    ...
    Mockito.verify(this.productLevel3Repository)
        .getItemNameByItemSku(Arrays.asList(this.messageEvent.getGdnSku()));
    Mockito.verify(this.productStockAlertService).updateMinimumStock(this.messageEvent, PRD_NAME, 0);
    Mockito.verify(this.productLevel3AggregatorService).updateMinimumStock(this.messageEvent, true, 0);
    Mockito.verify(this.eventStoreKafkaMessageProcessor).process(this.messageEvent,
        DomainEventName.STOCK_MINIMUM_STOCK_ALERT_NAME);
  }
  @Test
  public void testOnDomainEventConsumed_PPTest() throws Exception {
    Mockito
      .when(this.productLevel3Repository
        .getItemNameByItemSku(Arrays.asList(this.messageEvent.getGdnSku())))
      .thenReturn(itemNames);
