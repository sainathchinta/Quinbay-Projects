...
public class EditedImageResizeKafkaSubscriberBeanTest {
  ...
  @InjectMocks
  private EditedImageResizeKafkaSubscriberBean editedImageResizeKafkaSubscriberBean;
  ...
  public void onDomainEventConsumed() throws Exception {
    Mockito.doNothing().when(imageProcessorService).resizeEditedImage(bulkResizeImageRequest);
    ...
    this.editedImageResizeKafkaSubscriberBean.onDomainEventConsumed(message);
    Mockito.verify(imageProcessorService).resizeEditedImage(bulkResizeImageRequest);
  ...
  public void onDomainEventConsumedExceptionTest() throws Exception {
    Mockito.doThrow(ApplicationRuntimeException.class).when(imageProcessorService).resizeEditedImage(bulkResizeImageRequest);
    String message = mapper.writeValueAsString(editedImageResizeEvent);
    Mockito.when(objectMapper.readValue(message, EditedImageResizeEvent.class)).thenReturn(editedImageResizeEvent);
    this.editedImageResizeKafkaSubscriberBean.onDomainEventConsumed(message);
    Mockito.verify(imageProcessorService).resizeEditedImage(bulkResizeImageRequest);
