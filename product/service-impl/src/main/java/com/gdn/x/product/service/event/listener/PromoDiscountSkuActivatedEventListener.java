...
public class PromoDiscountSkuActivatedEventListener {
  ...
  @Deprecated
  @KafkaListener(topics = DomainEventName.PROMO_ADJUSTMENT_ACTIVATION_EVENT, autoStartup = "#{kafkaTopicProperties.isAutoStartup()}")
  public void onDomainEventConsumed(String message) {
    ...
    try {
      ...
      GdnPreconditions.checkArgument(promoAdjustmentChangeEvent.isActivated(),
          ErrorMessages.PROMO_DISCOUNT_EVENT_STATUS_MUST_NOT_BE_INACTIVE);
      ...
      item.getPrice().forEach(price -> price.setMerchantPromoDiscountPrice(
          new DiscountPrice(promoAdjustmentChangeEvent.getValue(), promoAdjustmentChangeEvent.getStartDate(), promoAdjustmentChangeEvent.getEndDate(),
              promoAdjustmentChangeEvent.getPromoAdjustmentName(), AdjustmentType.MERCHANT)));
      this.itemService.updateItemMerchantDiscountPrice(promoAdjustmentChangeEvent.getStoreId(), item);
    } catch (Exception ex) {
      log.error(
          "Error while Event listening Promo-Discount Activation from Promo-Discount , payload: "
            + "{}, error - ", message, ex);
    }
  }
}
